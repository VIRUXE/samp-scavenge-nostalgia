...
static
				skl_Levels[5][E_SKILL_LEVEL_DATA] = {
					{"Novato",			C_YELLOW,	0.1},
					{"Adequado",		C_GREEN,	0.2},
					{"Intermediario",	C_ORANGE,	0.33},
					{"Experiente",		C_BLUE,		0.66},
					{"Mestre",			C_PINK,		0.9}
				};
...
skl_ShowSkillList(playerid)
{
	...
	for(new i; i < skl_PlayerSkillCount[playerid]; i++)
	{
		...
		format(gBigString[playerid], sizeof(gBigString[]), "%s%s%s\t%s\t(%0.1f / 100.0)\n",
				gBigString[playerid],
				skl_Levels[level][skl_colour],
				skillname,
				skl_Levels[level][skl_level],
				(skl_PlayerSkills[playerid][i][skl_amount] / skl_Levels[level][skl_bound]) * 100
			);
	}

	Dialog_Show(playerid, SkillsDialog, DIALOG_STYLE_TABLIST_HEADERS, "Skills - Suas habilidades",
		sprintf("Skill\tLevel\tProgresso\n%s", gBigString[playerid]), "Voltar", "");

	return 1;
}

Dialog:SkillsDialog(playerid, response, listitem, inputtext[])
