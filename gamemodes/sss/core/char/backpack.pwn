...
stock AddItemToPlayer(playerid, itemid, useinventory = false, playeraction = true)
{
	...
	{
		ShowActionText(playerid, sprintf(GetLanguageString(playerid, "common/empty"), itemsize - freeslots), 3000, 150);
		return -4;
	}
	...
	if(playeraction)
	{
	    if(IsValidItem(bag_PlayerBagID[playerid]))
	    {
			ShowActionText(playerid, GetLanguageString(playerid, "common/empty"), 3000, 150);
		...
		else ShowActionText(playerid, sprintf(GetLanguageString(GetPlayerLanguage(playerid), "common/empty"), itemsize), 3000, 150);
	}
...
hook OnPlayerSelectInvOpt(playerid, option)
{
	...
	if(IsValidItem(bag_PlayerBagID[playerid]) && !IsValidContainer(GetPlayerCurrentContainer(playerid)))
	{
		if(option == bag_InventoryOptionID[playerid])
		{
			...
			if(required > 0)
				ShowActionText(playerid, sprintf(GetLanguageString(playerid, "common/empty"), required), 3000, 150);
