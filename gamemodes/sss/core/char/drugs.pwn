...
stock ApplyDrug(playerid, drugtype, customduration = -1)
{
	...
	ShowActionText(playerid, sprintf(ls(playerid, "item/drugs/taken"), drug_TypeData[drugtype][drug_name]), 3000);
	...
	if(!IsPlayerNPC(playerid))
	{
		for(new i; i < drug_TypeTotal; i++)
		{
			if(drug_PlayerDrugData[playerid][i][drug_active])
			{
				...
				{
					ShowActionText(playerid, sprintf(ls(playerid, "item/drugs/worn-off"), drug_TypeData[i][drug_name]), 3000);
...
CMD:druginfo(playerid)
{
	...
	ShowPlayerDialog(playerid, 10008, DIALOG_STYLE_MSGBOX, "Drug Debug (Debrdug!?)", gBigString[playerid], "Fechar", "");
