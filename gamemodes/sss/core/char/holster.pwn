...
timer HolsterItemDelay[time](playerid, itemid, time)
{
	...
	if(IsValidItem(currentitem))
	{
		...
		switch(GetPlayerLanguage(playerid))
	    {
	         case 0:
	         {
	              ShowActionText(playerid, "Changed", 3000, 70);
	         }
	         case 1:
	         {
	              ShowActionText(playerid, "Trocado", 3000, 70);
	         }
    	}
	...
	else
	{
		switch(GetPlayerLanguage(playerid))
	    {
	         case 0:
	         {
	              ShowActionText(playerid, "Weapon put in the holster", 3000, 70);
	         }
	         case 1:
	         {
	              ShowActionText(playerid, "Arma colocada no coldre", 3000, 70);
	         }
    	}
	}
...
timer UnholsterItemDelay[time](playerid, time)
{
	...
	switch(GetPlayerLanguage(playerid))
    {
         ...
         case 1:
         {
              ShowActionText(playerid, "Equipado", 3000, 70);
         }
	}
...
{
	dbg("global", CORE, "[OnPlayerOpenInventory] in /gamemodes/sss/core/char/holster.pwn");

	if(GetTickCountDifference(GetTickCount(), hols_LastHolster[playerid]) < 2000)
		return Y_HOOKS_BREAK_RETURN_1;
...
{
	dbg("global", CORE, "[OnPlayerOpenContainer] in /gamemodes/sss/core/char/holster.pwn");
