...
{
	dbg("global", CORE, "[OnPlayerGiveDamage] in /gamemodes/sss/core/weapon/damage-melee.pwn");
...
_DoMeleeDamage(playerid, targetid, Float:bleedrate, Float:knockmult)
{
	...
	GetPlayerPos(playerid, px, py, pz);
	GetPlayerPos(targetid, ix, iy, iz);
	
    if(CA_RayCastLine(ix, iy, iz, px, py, pz, tmp, tmp, tmp))
    	return 0;
	...
	PlayerInflictWound(playerid, targetid, E_WOUND_MELEE, bleedrate, knockmult, NO_CALIBRE, random(2) ? BODY_PART_TORSO : BODY_PART_HEAD, "Arma branca");
