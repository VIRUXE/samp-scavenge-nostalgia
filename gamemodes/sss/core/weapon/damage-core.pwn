...
stock PlayerInflictWound(playerid, targetid, E_WND_TYPE:type, Float:bleedrate, Float:knockmult, calibre, bodypart, source[])
{
	...
	ShowActionText(targetid, sprintf(GetLanguageString(targetid, "common/empty"), source, (knockmult * (((woundcount + 1) * 0.2) * ((totalbleedrate * 50) + 1)) < 50.0 ? ("Baixa") : ("Alta"))), 5000);
...
stock SetLastHitById(playerid, lasthit)
{
	if(!IsPlayerConnected(playerid))
		return 0;

    dmg_LastHitById[playerid] = lasthit;
	return 1;
}
