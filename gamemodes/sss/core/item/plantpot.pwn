...
hook OnPlayerUseItemWithItem(playerid, itemid, withitemid)
{
	...
	if(GetItemType(withitemid) == item_PlantPot)
	{
		...
		if(GetItemTypeLiquidContainerType(itemtype) != -1)
		{
			...
			if(amount <= 0.0)
			{
				ShowActionText(playerid, GetLanguageString(GetPlayerLanguage(playerid), "POTBOTEMPTY", true), 5000);
			}
			else if(type != liquid_Water)
			{
				ShowActionText(playerid, GetLanguageString(GetPlayerLanguage(playerid), "POTBOTNOWAT", true), 5000);
			}
			else
			{
				...
				ShowActionText(playerid, GetLanguageString(GetPlayerLanguage(playerid), "POTADDWATER", true), 5000);
			}
		}
		...
		if(itemtype == item_Knife)
		{
			...
			{
				ShowActionText(playerid, GetLanguageString(GetPlayerLanguage(playerid), "POTNOACPLNT", true), 3000);
			...
			{
				ShowActionText(playerid, GetLanguageString(GetPlayerLanguage(playerid), "POTNOTGROWN", true), 3000);
				return Y_HOOKS_BREAK_RETURN_1;
			}
