...
_CatchFish(playerid, Float:distance)
{
	...
	{
		fish_Timer[playerid] = defer _TryCatch(playerid, MIN_FISHING_TIME + random(MAX_FISHING_TIME - MIN_FISHING_TIME));
		ShowActionText(playerid, ls(playerid, "item/fishingrod/started"));
	}
	else
	{
		_PlayerStopFishing(playerid);
		ShowActionText(playerid, ls(playerid, "item/fishingrod/move-closer"), 8000);
	}
...
timer _TryCatch[catchtime](playerid, catchtime)
{
	...
	if(random(1000) < 1000 - 1000 - FISHING_CHANCE)
	{
		ApplyAnimation(playerid, "SWORD", "sword_block", 50.0, 1, 0, 0, 0, 0);

		fish_Timer[playerid] = defer _CatchDelay(playerid);
		ShowActionText(playerid, ls(playerid, "item/fishingrod/line-pulling"), floatround(fish_Distance[playerid], floatround_round) * 100);
	}
	else
	{
		_PlayerStopFishing(playerid);
		ShowActionText(playerid, ls(playerid, "item/fishingrod/no-luck"), 8000);
	}
}
