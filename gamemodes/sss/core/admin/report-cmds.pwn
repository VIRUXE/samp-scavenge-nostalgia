...
ACMD:blockrr[1](playerid, params[]) {
	...
	return ChatMsg(playerid, YELLOW, RelatorioBlock[targetId] ? " > Você bloqueou %p de usar o /relatorio!" : " > %p agora pode usar /relatorio", targetId);
}
...
ACMD:rr[1](playerid, params[]) {
	...
	if(!RelatorioEnviado[targetId]) return ChatMsg(playerid, RED, " > Esse player não enviou nenhum relatório ou já foi respondido.");

    ChatMsg(targetId, GREEN, "="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"=");
	ChatMsg(targetId, GREEN, sprintf("[Relatório] %p(id:%d) respondeu: "C_WHITE"%s", playerid, playerid, msg));
	ChatMsg(targetId, GREEN, "="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"="C_WHITE"="C_GREEN"=");

	ChatMsgAdmins(1, GREEN, sprintf(" > %p(id:%d) respondeu o relatório de %p(id:%d): %s", playerid, playerid, targetId, targetId, msg));
...
CMD:report(playerid) {
    if(GetPlayerAdminLevel(playerid) > 1) return ChatMsg(playerid, RED, " > Você não pode usar este comando.");
...
ACMD:reports[1](playerid, params[]) {
	if(!ShowListOfReports(playerid)) ChatMsg(playerid, YELLOW, " >  Não existem reports.");
...
ShowReport(playerid, reportlistitem) {
	...
	Dialog_Show(playerid, Report, DIALOG_STYLE_MSGBOX, report_CurrentReportList[playerid][reportlistitem][report_name], message, "OpÃ§Ãµes", "Voltar");
...
ShowReportOptions(playerid) {
	...
	if(IsPlayerOnAdminDuty(playerid) && GetPlayerAdminLevel(playerid) >= LEVEL_LEAD) {
		...
		if(isequal(report_CurrentType[playerid], "CAM")) {
			strcat(options, "Ir para o local da cÃ¢mera\n");
			strcat(options, "Ver a cÃ¢mera\n");
		}
			...
			case 6: {
				if(isequal(report_CurrentType[playerid], "CAM")) {
					if(IsPlayerOnAdminDuty(playerid)) {
						...
						ChatMsg(playerid, YELLOW, " >  Use /recam para resetar sua cÃ¢mera");
					}
				}
			}
		}
...
ShowReportBanPrompt(playerid) {
	...
	Dialog_Show(playerid, BanPrompt, DIALOG_STYLE_INPUT, "Insira a duraÃ§Ã£o do banimento", "Digite a duraÃ§Ã£o do banimento abaixo, insira o número e o tempo. Exemplo: '1 days': 'days', 'weeks' ou 'months'. Escreva 'forever' para um ban permanente.", "Continar", "Cancelar");
