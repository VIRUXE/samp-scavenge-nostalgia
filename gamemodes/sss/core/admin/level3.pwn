...
ACMD:veiculo[3](playerid, params[]) {
	...
	if(isequal(command, "puxar", true)) {
		...
		new occupants[4] = {INVALID_PLAYER_ID, ...}; // 4 � o máximo de jogadores que podem estar em um Ve�culo
		...
		foreach(new i : Player) {
			if(GetPlayerVehicleID(i) == vehicleid) {
				...
				occupants[seat] = i; // Armazenamos o jogador de acordo com a sua Posi��o no Ve�culo
		...
		SetVehicleHealth(vehicleid, 990.0); // Não podemos reparar o Ve�culo mais do que 990.0. Mais do que isso � hack.
...
ACMD:comandoslvl3[3](playerid) {
    ...
    strcat(stringlvl3, ""C_BLUE"/spec /free - Observar algu�m, camera livre\n");
    ...
    strcat(stringlvl3, ""C_BLUE"/resetarsenha - Resetar senha de algu�m (a senha nova será: 'password')\n");
