...
ACMD:field[2](playerid, params[])
{
	...
	if(!strcmp(params, "log", true, 3))
	{
		...
		else ChatMsg(playerid, YELLOW, " >  N„o h√° log de entradas na field: '%s'.", name);
	}
	...
	if(!strcmp(params, "nome", true, 4))
	{
		...
		if(count == 0) ChatMsg(playerid, YELLOW, " >  N„o h√° registro de field encontradas em: '"C_BLUE"%s"C_YELLOW"'.", name);
	}
...
Dialog:DetfieldRename(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(ret == -2)
			ChatMsg(playerid, RED, " >  Nome de field inv√°lida. Deve come√ßar com um caractere alfabÈtico e pode conter apenas caracteres alfanumÈricos.");
	}
...
AddNewDetectionFieldPoint(playerid)
{
	...
	else ChatMsg(playerid, YELLOW, " >  Ponto %d setado para %f, %f. Mova para o pr√≥ximo ponto e pressione "C_BLUE"~k~~PED_LOCK_TARGET~", dfm_CurrentPoint[playerid] + 1, dfm_Points[playerid][dfm_CurrentPoint[playerid] * 2], dfm_Points[playerid][(dfm_CurrentPoint[playerid] * 2) + 1]);
