...
{
    if(!IsPlayerLoggedIn(playerid)) return SendClientMessage(playerid, RED, " > Voc� precisa estar logado para isso.");
    if(GetPlayerScore(playerid) < 100 && !IsPlayerVip(playerid)) return SendClientMessage(playerid, RED, " > Voc� precisa ter no m�nimo 100 score para usar este comando.");
