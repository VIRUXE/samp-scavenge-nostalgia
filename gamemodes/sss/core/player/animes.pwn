...
CMD:animes(playerid)
{
	ChatMsg(playerid,RED,"Lista de anima��es -");
...
//    ShowActionText(playerid, "~>~ ~w~Use ~r~/stop~w~ para parar.");
    ...
    ChatMsg(playerid, RED, " > Use /stop para parar a anima��o.");
    ChatMsg(playerid, RED, " ");
	return 1;
}
...
CMD:stop(playerid)
{
	if(GetPlayerTotalVelocity(playerid) > 0.0){
	    SendClientMessage(playerid, RED, " > Voc� deve estar parado para usar este comando.");
	    return 1;
	}
	
    ClearAnimations(playerid);
    ShowActionText(playerid, "Anima�o resetada.");
