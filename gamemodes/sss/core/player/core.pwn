	...
	if(!localhost) {
		...
		switch (hashStatus) {
			case 0: { // Hash recorded multiple times but not allowed
				...
				ChatMsgAdmins(LEVEL_DEVELOPER, WHITE, "%s foi kickado por ter v·rias contas.", playerName);
			...
			case 1: { // Hash recorded multiple times and allowed
				...
				ChatMsgAdmins(LEVEL_DEVELOPER, WHITE, "%s tem v·rias contas, mas est· autorizado.", playerName);
			}
			case 2: { // Hash is unique
				...
				ChatMsgAdmins(LEVEL_DEVELOPER, WHITE, "%s tem um hash ˙nico.", playerName);
			}
			case 3: {
				...
				ChatMsgAdmins(LEVEL_DEVELOPER, WHITE, "%s n„o tem hash registrado.", playerName);
			}
			case -1: { // Invalid hash format
				...
				ChatMsgAdmins(LEVEL_DEVELOPER, WHITE, "%s tentou entrar com um hash inv·lido.", playerName);
			...
			case -3: { // Error occurred during query execution
				log("[GPCI] Error executing query for %s.", playerName);
			...
			default: {
				log("[GPCI] Something wrong happened with %s. Return: %d.", playerName, hashStatus);

				KickPlayer(playerid, "Error: Please contact an admin.");

				ChatMsgAdmins(LEVEL_DEVELOPER, WHITE, "Ocorreu um erro no CheckPlayerHashStatus para %s. Return: %d.", playerName, hashStatus);
		...
		ChatMsgAdmins(LEVEL_LEAD, WHITE, "%p (%d) est· a utilizar a vers„o '%s'.", playerid, playerid, version);
