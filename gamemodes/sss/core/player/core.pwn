	...
	if(!localhost) {
		...
		switch (CheckPlayerHashStatus(playerName, hash)) {
			case 0: { // Hash recorded multiple times but not allowed
				...
				ChatMsgAdmins(LEVEL_MODERATOR, WHITE, "%s foi kickado por ter v·rias contas.", playerName);
		...
		ChatMsgAdmins(LEVEL_ADMINISTRATOR, WHITE, "%p (%d) est· a utilizar a vers„o '%s'.", playerid, playerid, version);
