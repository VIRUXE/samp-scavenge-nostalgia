...
hook OnPlayerEnterDynArea(playerid, areaid)
{
	...
	if(IsValidItem(data[1]))
	{
		if(GetItemType(data[1]) == item_PlotPole)
		{
			...
			ShowActionText(playerid, sprintf(GetLanguageString(playerid, "PLOTPOLEENT", true), geid), 5000);
		}
	}
...
hook OnPlayerLeaveDynArea(playerid, areaid)
{
	...
	if(IsValidItem(data[1]))
	{
		if(GetItemType(data[1]) == item_PlotPole)
		{
			new geid[GEID_LEN];
			GetItemGEID(data[1], geid);
			ShowActionText(playerid, sprintf(GetLanguageString(playerid, "PLOTPOLELEF", true), geid), 5000);
		}
	}
