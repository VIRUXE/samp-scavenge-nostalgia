    ...
    if(dialogid == DialogComerciante)
	{
		if(response)
		{
			...
			switch(listitem)
			{
				case 0:
				{
                    ShowPlayerDialog(playerid, DialogCompra, DIALOG_STYLE_TABLIST_HEADERS, "Compra de Itens:", "Itens\tPreÃ§o\tQuantidade\n\
                    Camuflagem\t30 Coins\t1\n\
                    Sinalizador\t25 Coins\t1\n\
					EMP de Proximidade\t25 Coins\t1\n\
                    Teclado AvanÃ§ado\t20 Coins\t1\n\
                    Gazua EletrÃ´nica\t20 Coins\t1\n\
                    Molotov\t15 Coins\t1\n\
				...
				case 1:
				{
                    ShowPlayerDialog(playerid, DialogVenda, DIALOG_STYLE_TABLIST_HEADERS, "Venda de Itens:", "Item\tPreÃ§o\n\
                    Corpo\t1 Coin\n\
                    PedaÃ§o de Metal\t1 Coin", ""C_GREEN">", ""C_RED"X");
				}
				case 2: 
				{
			...
			switch(listitem)
			{
				case 0:
				{
					if(GetPlayerCoins(playerid) < 30) {
						dialogOpen[playerid] = false;
						return SendClientMessage(playerid, RED, " > Você nÃ£o tem coins o suficiente."); 
					}   
				...
				case 1:
				{
					if(GetPlayerCoins(playerid) < 25) {
						dialogOpen[playerid] = false;
						return SendClientMessage(playerid, RED, " > Você nÃ£o tem coins suficiente.");
					}		
			...
			switch(listitem)
			{
				case 0:
				{
					if(itemtype == item_Torso)
					{
						...
						ChatMsg(playerid, RED, " > Você nÃ£o tem o item selecionado em mÃ£os para vender.");
						dialogOpen[playerid] = false;
					}
				}
				case 1:
...
stock BuyItem(playerid, item, value){
	...
	{
		ChatMsg(playerid, YELLOW, " >  Aguarde no mÃnimo 1 segundo para interagir com o comerciante novamente.");
		return 1;
	}
