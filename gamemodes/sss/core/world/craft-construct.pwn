...
hook OnPlayerUseItem(playerid, itemid)
{
	...
	if(size > 1)
	{
		...
		for(new i; i < size; i++)
		{
		    if(IsItemTypeDefence(GetItemType(list[i])) && GetDefenceActive(list[i]))
				continue;
				...
				if(cons_Data[cons_CraftsetConstructSet[craftset]][cons_tool] == GetItemType(GetPlayerItem(playerid)))
				{
					...
					if(!CallLocalFunction("OnPlayerConstruct", "dd", playerid, cons_CraftsetConstructSet[craftset]))
					{
						...
						if(IsPlayerVip(playerid))
							StartHoldAction(playerid, cons_Data[cons_CraftsetConstructSet[craftset]][cons_buildtime] / 3);
						else
...
StartRemovingConstructedItem(playerid, itemid, craftset)
{
	...
	if(IsPlayerVip(playerid))
		StartHoldAction(playerid, cons_Data[cons_CraftsetConstructSet[craftset]][cons_removalTime] / 3);
	else
	    StartHoldAction(playerid, cons_Data[cons_CraftsetConstructSet[craftset]][cons_removalTime]);
	    
	ApplyAnimation(playerid, "BOMBER", "BOM_Plant_Loop", 4.0, 1, 0, 0, 0, 0);
	ShowActionText(playerid, ls(playerid, "DECONSTRUCT"));
