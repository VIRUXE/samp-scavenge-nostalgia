...
_rm_PlayerUseRefineMachine(playerid, itemid, interactiontype)
{
	...
	if(data[rm_cooking])
	{
		ShowActionText(playerid, sprintf(GetLanguageString(GetPlayerLanguage(playerid), "common/empty"), MsToString(data[rm_cookTime] - GetTickCountDifference(GetTickCount(), data[rm_startTime]), "%m minutes %s seconds")), 8000);
		return 0;
	}
...
Dialog:RefineMachine(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(ret == 0)
			...
			ShowActionText(playerid, sprintf(GetLanguageString(playerid, "common/empty"), REFINE_MACHINE_FUEL_USAGE), 6000);
