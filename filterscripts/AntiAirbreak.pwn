...
public OnFilterScriptInit(){
    for(new i = 0; i < MAX_PLAYERS; i++)
	    if(IsPlayerConnected(i))
			GetPlayerPos(i, ab_SetX[i], ab_SetY[i], ab_SetZ[i]);
	return 1;
}

public OnPlayerConnect(playerid)
    ab_Check[playerid] = false;
...
public OnIncomingPacket(playerid, packetid, BitStream:bs){
	...
	if(packetid == 207){
	    ...
	    return ab_PosCheck(playerid,
			data[PR_position][0], data[PR_position][1],data[PR_position][2]);
	}
	// DRIVER_SYNC
	if(packetid == 200){
...
// Verifica a posi��o do jogador e compara com a antiga;
ab_PosCheck(playerid, Float:x, Float:y, Float:z)
{
	if(!IsPlayerConnected(playerid))
	    return 0;
	    
	if(ab_ChangePosTick[playerid] > gettime())
	    return 1;
	    
 	if(ab_SetX[playerid] == x && ab_SetY[playerid] == y && ab_SetZ[playerid] == z)
	    return 1;
