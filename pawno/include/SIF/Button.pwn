...
forward CreateButton(Float:x, Float:y, Float:z, text[], world = 0, interior = 0, Float:areasize = 1.0, label = 0, labeltext[] = "", labelcolour = 0xF4F98EFF, Float:streamdist = BTN_DEFAULT_STREAMDIST, testlos = true);
/*
# Description
...
forward SetButtonLabel(buttonid, text[], colour = 0xF4F98EFF, Float:range = BTN_DEFAULT_STREAMDIST, testlos = true);
/*
# Description
...
hook OnScriptInit()
{
	...
	#if defined DEBUG_LABELS_BUTTON
		btn_DebugLabelType = DefineDebugLabelType("BUTTON", 0xF4F98EFF);
	#endif
}
...
stock CreateButton(Float:x, Float:y, Float:z, text[], world = 0, interior = 0, Float:areasize = 1.0, label = 0, labeltext[] = "", labelcolour = 0xF4F98EFF, Float:streamdist = BTN_DEFAULT_STREAMDIST, testlos = true)
{
...
// btn_label
stock SetButtonLabel(buttonid, text[], colour = 0xF4F98EFF, Float:range = BTN_DEFAULT_STREAMDIST, testlos = true)
{
	...
	if(!IsPlayerInAnyVehicle(playerid) && Iter_Count(btn_NearIndex[playerid]) < BTN_MAX_INRANGE)
	{
		...
		if(data[0] == BTN_STREAMER_AREA_IDENTIFIER)
		{
			...
			if(Iter_Contains(btn_Index, data[1]))
			{
				...
				ShowActionText(playerid, "Pressione ENTER para interagir");
