...
stock GetItemArrayDataAtCell(itemid, cell)
{
	...
	if(cell > itm_arr_ItemTypeArrayMax[itemtype])
	{
		...
		printf("[GetItemArrayDataAtCell] ERROR: Cell %d exceeds item type (%d) max of %d. ID: %d - %0.1f, %0.1f, %0.1f", cell, _:itemtype, itm_arr_ItemTypeArrayMax[itemtype], itemid, x, y, z);
		return -1;
	}
