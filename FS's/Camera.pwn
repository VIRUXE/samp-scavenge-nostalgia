...
CMD:savecamera(playerid, params[])
{
	...
	if(GetPVarType(playerid, "FlyMode"))
		...
		SendClientMessage(playerid, -1, "Voc� n�o pode usar este comando agora");
	return 1;
}
...
stock SaveCamera(playerid, nome[])
{
	...
	format(str, sizeof str, "A posi��o da camera '{FFFFFF}%s{F6F600}' foi salva no arquivo ({FFFFFF}%s{F6F600}) em sua pasta 'scriptfiles'",nome, PASTA_SAVE);
...
{
    SendClientMessage(playerid, 0xF6F600FF, "Voc� foi spawnado.");
